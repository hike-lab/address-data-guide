...
> 데이터 품질에 대한 다양한 정의와 해석이 존재하며, 개별 품질 요소에 대한 일관적인 평가 체계가 미흡한 상황이다. '데이터 품질'과 '품질 평가 요소'에 대한 전반적인 정의와 설명은 다수의 참고 문헌과 공공데이터 관리지침에서 제시된 테이블 형식의 파일 데이터의 품질 요소와 정의, 평가 방식을 참고하여 인용한다.  관련 자료는 모두 각주로 첨부되어 있다. 
