...
<details>
<summary>도로명주소 관련지번 레이아웃</summary>
...
해당 데이터는 동,층,호의 구분을 위해 일련번호을 적용해 동일련번호, 층일련번호, 호일련번호, 호접미사일련번호로 통해 관리합니다. 이 때, 호접미사는 호에서 상세구분을 하는 경우(예: 101호 A, 101호 B)에 사용합니다.

다른 데이터와 연계하기 위해 건물관리번호, 법정동코드, 도로명코드 등 도로명주소와 관련된 정보를 함께 제공합니다.
...
데이터 레이아웃은 같지만, 건물명의 기준에 따라 동,층,호의 정보가 변경되므로 2개의 데이터를 별도로 제공합니다. 예를 들어, 건축물대장의 동,층,호 정보가 없지만 이용자의 요청에 따라 시군구용 상세주소에는 동,층,호가 입력될 수 있습니다.
...
도로명주소는 건물군 또는 건물에 부여됩니다. 만약, 건물군에 여러 개의 건물이 포함되어 있을 경우, 상세주소 동 표시 데이터를 사용해 건물 동단위로 부여된 도로명주소를 확인할 수 있습니다. 층과 호는 제외하고 건물단위로 제공하므로 '상세주소 표시' 데이터와 다르게 동,층,호 정보가 없습니다.
...
<details>
<summary>상세주소 동 표시 레이아웃</summary>
...
<details>
<summary>사물주소 버스정류장 마스터 레이아웃</summary>
...
현재의 공개하는 주소는 2022년 6월 9일 이후 기존 9종에서 17종으로 세분화하여 제공합니다.  
공개하는 주소 상단의 버튼을 통해 **공개하는 정보 변동사항 안내**를 확인할 수 있습니다. 
...
이전 DB와 현행 DB의 구조가 다르므로, 이전 DB와 현행 DB를 연계하여 사용하려면 데이터를 재구성해야 합니다.