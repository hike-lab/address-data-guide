---
title: 3.3 공개하는 주소
description: 주소정보 제공의 공개하는 주소는 도형정보 또는 좌표를 제외한 자료로 구성되어 있다. 이용자는 로그인 절차없이 누구나 이용할 수 있다.
keywords:
  [
    "주소정보누리집",
    "주소데이터",
...
주소정보 제공의 [공개하는 주소](https://business.juso.go.kr/addrlink/adresInfoProvd/guidance/othbcAdresInfo.do)는 도형정보 또는 좌표를 제외한 자료로 구성되어 있다. 이용자는 로그인 절차없이 누구나 이용할 수 있다.

<figure class="flex flex-col items-center justify-center">
    <img src="../img/open-address.png" title="공개하는주소">
    <figcaption style="text-align: left; font-size: 10pt;">출처: 주소정보 누리집</figcaption>
</figure>

공개하는 주소는 6종으로 구성되어 있고, 주소정보 활용 시 도로명주소법 시행령 6조에 따른 구성과 표기방법을 준수해야한다.
...
각 데이터는 전체자료, 월변동자료, 일변동자료로 구분되어 있고, 목적에 따라 이용할 수 있다. 일변동자료를 지속적으로 업데이트해야하는 경우, 연계신청을 통해 API 또는 관련 프로그램을 통해 제공받을 수 있다.

<figure class="flex flex-col items-center justify-center">
    <img src="../img/open-address-detail.png" title="공개하는주소">
    <figcaption style="text-align: left; font-size: 10pt;">출처: 주소정보 누리집</figcaption>
</figure>

공개하는 주소의 데이터는 대부분 TXT 파일이며, 구분자는 `|`, 인코딩은 `CP949`로 제공한다.  
데이터 내에서 컬럼명이 제공되지 않으므로, 데이터를 분석하기 전에 데이터의 구조를 파악해야한다. 각 데이터의 스키마은 '공개하는 주소'의 팝업창에서 확인할 수 있고, 상세한 활용가이드는 '활용가이드다운로드' 버튼을 통해 PDF로 제공하고 있다.

<br>
<figure class="flex flex-col items-center justify-center">
    <img src="../img/DB-guide.png" title="활용가이드다운로드">
    <figcaption style="text-align: left; font-size: 10pt;">출처: 주소정보 누리집</figcaption>
</figure>
<br>
...
도로명주소 한글 데이터는 각 컬럼을 조합하여 약 600만여건의 도로명주소를 구성할 수 있다. 여러 개의 건물이 하나의 도로명주소를 갖는 경우(집합건물, 아파트) 한 건의 주소정보를 제공한다.

2개의 데이터로 구성되어 있으며, 전체분과 관련지번으로 구분된다. 다운로드시 압축파일(zip)로 제공하고, 전체분은 시도별로 구분되어 있다.
...
**도로명주소 한글**은 조합된 주소를 제공하지 않고, 각 컬럼을 조합하여 주소를 구성해야한다. ORACLE 기준의 SQL 쿼리를 제공하고 있으며, 이를 참고하여 주소DB를 구성할 수 있다. 본 문서는 범용적인 사용을 위해 [6장 파이썬과 MySQL로 구축하는 주소 데이터베이스](/contents/chapter-6/chapter-6-1.md)에서 MySQL로 구축하는 방법을 제공한다.
...
**관련지번**은 도로명주소와 N:M 관계로, 도로명주소에 여러 개의 지번이 포함될 수 있다.  
예를 들어 '서울특별시 동작구 흑석로 84 (흑석동)'은 건물군 단위로 주소가 부여되어, 부지가 크기 때문에 '흑석로 84'에 '흑석동' 지번이 여러 개 포함될 수 있다.

<figure class="flex flex-col items-center justify-center">
    <figcaption style="text-align: center;">< 관련지번 예시 ></figcaption>
    <img src="../img/related-jibun.png" title="관련지번">
    <figcaption style="text-align: left; font-size: 10pt;">출처: 주소정보 누리집</figcaption>
</figure>
...
도로명주소 영문 표기를 위해 제공하는 데이터다. 도로명주소 한글과 다르게 행정동코드, 행정동명 정보가 생략되어있고, 관련지번 데이터를 제공하지 않는다.
...
상세주소는 도로명주소의 건물번호 뒤에 표시되는 동·층·호 정보로, 2가구 이상 거주하는 원룸·다가구·단독주택에 부여한다.

해당 데이터는 동,층,호의 구분을 위해 동일련번호, 층일련번호, 호일련번호, 호접미사일련번호를 통해 관리한다. 이 때, 호접미사는 호에서 상세구분을 하는 경우(예: 101호 A, 101호 B)에 사용한다.
...
상세주소 데이터는 **시군구용 상세주소**와 **건축물대장 상세주소**로 구분되어 있다.  
데이터 스키마은 같지만, 건물명의 기준에 따라 동,층,호의 정보가 변경되므로 2개의 데이터를 별도로 제공한다. 예를 들어, 건축물대장 상세주소는 동,층,호 정보가 없지만, 시군구용 상세주소는 이용자의 요청에 따라 변경될 수 있으므로 동,층,호 정보가 추가될 수 있다.
...
도로명주소는 건물군 또는 건물에 부여된다. 만약, 건물군에 여러 개의 건물이 포함되어 있을 경우, 상세주소 동 표시 데이터를 사용해 건물 동단위로 부여된 도로명주소를 확인할 수 있다. 층과 호는 제외하고 건물단위로 제공하므로 '상세주소 표시' 데이터와 다르게 층,호 정보가 없다.
...
도로명주소관리번호와 건물관리번호를 PK로 사용하여 도로명주소와 연계할 수 있다. 이 때, 상세건물명은 건물군 내 건물을 구분하기 위해 사용한다. 아래 예시의 경우, '우남이채롬'이 건축물대장 건물명이고, '101동', '102동', '103동'이 상세건물명이다.
...
사물주소는 도로명과 기초번호를 사용하여 시설물에 부여한 주소다. 총 15종의 시설물에 부여한 주소데이터를 제공한다. (단, 육교승강기는 승강기, 출입구, 육교로 구분하여 제공한다.)
...
사물주소는 '주소'와 '마스터'로 구분된다. '주소'는 사물주소의 행정구역, 도로명, 주소본번, 주소부번, 대표주소여부 등 주소와 관련된 데이터를 제공하고, '마스터'는 시설물의 유형에 따른 상세한 정보를 제공한다.

예를 들어, 버스정류장의 경우 '주소'에는 버스정류장의 사물주소를 제공하고, '마스터'에는 버스정류장의 상세한 정보(예:중앙차로 여부)를 제공한다.
...
마스터의 상세한 스키마은 시설물 유형마다 다르므로, [사물주소활용가이드 PDF](https://business.juso.go.kr/addrlink/download.do?reqType=jusuan_ex&fileName=%EC%82%AC%EB%AC%BC%EC%A3%BC%EC%86%8C%ED%99%9C%EC%9A%A9%EA%B0%80%EC%9D%B4%EB%93%9C.pdf&realFileName=jusuan_exDBGuide.pdf&regYmd=2023)를 참고하는 것을 권장한다.

## 도로명

도로명주소에 부여되는 도로명과 해당 도로명이 지나는 읍면동, 도로구간의 기점과 종점을 제공한다.  
도로명은 지역별로 구분되지 않고 하나의 데이터로 제공된다.
...
도로명은 시군구코드와 도로명번호를 PK로 사용한다. **도로명번호 7자리는 지자체마다 관리하는 코드로, 고유하지 않다.** 따라서, 시군구코드와 도로명번호를 조합한 코드(도로명코드)를 통해 도로명을 식별한다. 하지만, 데이터에 따라 도로명번호(7자리)와 도로명코드(12자리)의 용어가 혼용되므로 자릿수에 주의해야 한다.
...
현재의 공개하는 주소는 2022년 6월 9일 이후 기존 9종에서 17종으로 세분화하여 제공하고 있다.
...
<figure class="flex flex-col items-center justify-center">
    <img src="../img/DB-notice.jpg" title="DB변동사항">
    <figcaption style="text-align: left; font-size: 10pt;">출처: 주소정보 누리집</figcaption>
    
</figure>

이전의 데이터 중 일부 DB는 공개하는 주소와 함께 제공된다.

| DB명             | 설명                                                                                                                                                                                                                                                                                                |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 건물DB           | 도로명주소를 구성하는 기본 단위인 건물정보와 해당 건물이 위치한 토지(지번)정보로 구성된 DB다. 하나의 도로명주소가 부여된 단지형 아파트의 경우에도 상세 동별 정보가 모두 제공되므로, 건물 단위의 주소 활용에 용이한다.                                                                               |
| 주소DB           | 주소DB는 여러 개의 건물이 하나의 도로명주소를 갖는 집합 건물(예: 아파트)의 경우 한 건의 주소정보를 제공하도록 구성된 주소 단위의 DB다. 600만여 건의 주소와 800만여 건의 지번정보를 바탕으로 사용자의 필요에 따라 선택적으로 활용할 수 있도록 도로명코드 / 주소 / 지번 / 부가정보로 분리·구성하였다. |
| 사서함주소DB     | 사서함주소DB는 인터넷 우체국(www.epost.kr)에 정기적으로 게시되는 사서함DB를 가공하여 제공한다.                                                                                                                                                                                                      |
| 영문도로명주소DB | 주소DB와 관련지번을 결합한 단일 파일로 구성하여 1개월 주기로 제공한다. (주소검색화면 구성 시 건물명은 사용자가 입력하도록 구성하시기 바랍니다.)                                                                                                                                                     |
| 상세주소DB       | 주소정보와 건물단위로 매칭가능한 상세주소정보를 제공한다. 상세주소란 도로명주소의 건물번호 뒤에 표시되는 동·층·호 정보로, 2가구 이상 거주하는 원룸·다가구·단독주택에 부여한다.                                                                                                                      |
| 사물주소DB       | 안전 사고 발생 시 신속 대응 지원 및 내비게이션 등에 안내체계 마련을 위해 도로명과 기초번호를 이용하여 구성한 사물주소정보를 제공한다. 사물주소란, 주소부여가 어려운 곳에 도로명과 기초번호를 이용하여 사물에 부여한 주소를 말한다.                                                                  |

본 문서는 현행 DB를 기준으로 작성하고 있으며, 이전 DB와 현행 DB를 연계하는 것을 원한다면 데이터를 재구성해야 한다.
