...
비어 있는 행을 확인하고, 각 컬럼별로 고유한 값의 개수를 살펴봅니다. '도로명주소관리번호'의 경우, 고유값은 전체 행 수와 동일한 6384988개입니다. 다시 말해, 도로명주소 데이터는 도로명주소관리번호를 기준으로 분석할 수 있음을 나타냅니다.

## 분석 1. 행정구역 기준 도로명주소 개수
...
위 코드는 우선 필요한 컬럼인 '시도명', '시군구명', '도로명', '도로명코드'를 추출한 뒤, 중복으로 인한 행을 제거합니다. 이후 '도로명'을 기준으로 그룹화하여 '도로명코드'의 개수를 확인합니다. 이를 내림차순으로 정렬하면, '중앙로'에는 93개의 상이한 도로명코드가 부여되어 있고 '신촌길'에는 50개의 상이한 도로명코드가 부여되어 있는 것을 확인할 수 있습니다.
