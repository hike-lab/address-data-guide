---
title: 3.4 제공하는 주소
description: 주소정보 제공의 제공하는 주소는 도형정보 또는 좌표를 포함한 자료로 구성되어 있다. 제공하는 주소의 유형과 활용 방법을 알아봅니다.
keywords:
  [
    주소정보누리집,
    출입구정보,
...
url: "/chapter-2/chapter-2-4.html"
---
...
[제공하는 주소](https://business.juso.go.kr/addrlink/adresInfoProvd/guidance/provdAdresInfo.do)는 도형정보 또는 좌표를 포함한 자료로 구성되어 있다. 총 11종의 데이터로 구성되어 있으며, 데이터 목록은 아래와 같다.
개별 데이터는 하나로 구성되어 있는 경우도 있지만, 관련 데이터와 함께 제공되는 경우도 있다. 예를 들어, 도로명주소 건물 도형 데이터는 출입구, 연결선 등의 정보를 포함하고 있다.
...
제공하는 주소는 공간데이터(SHP) 또는 좌표데이터(TXT)로 제공된다. 이번 장에서 공간데이터는 시각화를 통해 데이터를 설명하고, 좌표데이터는 예시를 통해 데이터를 설명한다. 각 데이터의 세부 내용은 제공하는 주소의 데이터 스키마와 활용가이드문서를 참고한다.

## 도로명주소 출입구 정보

도로명주소관리번호를 기준으로 출입구 정보를 제공한다. 집합건물의 경우 출입구는 주소기준의 주(보조)출입구를 제공하며, 건물단위 제공은 하지 않는다.

출입구는 좌표정보를 포함한 `TXT` 파일로 제공된다.
...
도로명주소 건물 도형은 건물의 폴리곤, 출입구 좌표, 연결선 라인 정보를 제공한다. 이때, 집합건물의 경우 건물군 단위의 도형정보를 제공하며, 건물단위의 도형정보는 제공하지 않는다. 모든 데이터는 `SHP` 파일로 제공된다. 데이터는 시도별로 제공된다.
...
<figure class="flex flex-col items-center justify-center">
    ...
    <img src="../img/building-qgis.png" title="건물도형">
</figure>

위 그림은 서울특별시의 일부 지역을 시각화한 결과다. 가장 옅은 회색으로 표시된 도형은 건물도형 정보이고, 회색 점은 건물 또는 건물군의 출입구 좌표다. 빨간색은 연결선이다. 연결선은 주소 부여대상과 참조대상 간을 서로 연결하는 선형을 말한다. 즉, 그림에서 주소가 부여된 건물의 출입구와 도로의 기초간격을 연결하는 선형을 말한다.

## 건물군 내 상세주소 동 도형

집합건물(예: 아파트) 내 동이 부여된 도로명주소의 도형정보를 제공한다.
동 도형은 Polygon 정보이므로, `SHP` 파일로 제공된다. 데이터는 시도별로 제공된다.
...
<figure class="flex flex-col items-center justify-center">
    <figcaption style="text-align: center;"></figcaption>
    <img src="../img/detail-building-qgis.png" title="건물군 내 동 도형">
</figure>

위 그림은 서울특별시의 일부 지역을 시각화한 결과다. 회색은 도로명주소의 건물도형 정보로, 건물군 단위의 도형만 제공한다. 반면, 갈색으로 표시된 도형은 건물군 내 상세주소 동 도형이다. 건물 도형에서 표현되지 않는 동 단위의 도형정보를 제공한다. 또한, 빨간색 점은 건물군 내 상세주소 동의 출입구 좌표다.

## 사물주소 시설 기준점

사물주소 15종의 시설물의 기준점을 제공한다. 기준점은 도로명과 기초번호를 이용하여 구성한 데이터로, `TXT` 파일로 제공된다.  
주소의 위치를 나타내는 주소 데이터는 X,Y 좌표를 포함하고 있다. 반면, 마스터 데이터는 좌표를 포함하지 않는다. 데이터는 시설물 유형별로 제공된다.
...
도로명과 기초번호를 이용하여 구성한 사물주소정보의 도형정보를 제공한다. 사물주소는 도형정보를 포함한 `SHP` 파일로 제공된다. 데이터는 시설물 유형별로 제공된다.

시설물 유형에 따라 점, 선, 면의 종류가 다르게 제공된다. 또한, 2024년 1월 기준으로 버스정류장과 택시승강장의 도형정보는 제공되지 않는다.
...
<figure class="flex flex-col items-center justify-center">
    <figcaption style="text-align: center;"></figcaption>
    <img src="../img/thing-qgis.png" title="사물주소 시설 도형">
</figure>

위 그림은 서울특별시의 지진옥외대피장소 예시다. 지진옥외대피장소는 점, 선, 면을 모두 제공한다. 주황색 면은 지진옥외대피장소의 면, 갈색 점은 지진옥외대피장소의 출입구, 빨간색 선은 출입구와 면을 연결하는 연결선이다.

## 기초번호

기초번호는 도로구간을 일정한 기초간격으로 나누어 부여한 번호다. 기초번호 데이터는 도로명주소의 기초번호를 기준으로 도로구간의 중심점 좌표정보를 제공한다. 기초번호는 `TXT` 파일로 제공된다. 모든 지역의 기초번호를 하나의 파일로 제공하고 있다.
...
국가지점번호는 도로명이 부여되지 않는 국토 또는 이와 인접한 해양 등의 위치표시쳬계 보완을 위해 사용한다. 국가지점번호 중심점은 전국을 일정한 격자 단위로 구획하여 부여한 지점의 번호와 해당 격자의 중심점 좌표를 제공한다. 국가지점번호 중심점은 `TXT` 파일로 제공되고, 하나의 파일로 제공하고 있다.
...
국가지점번호를 100KM, 10KM, 1KM, 100M 단위로 나누어 도형정보를 제공한다. 국가지점번호 도형은 `SHP` 파일로 제공된다. 데이터는 격자단위별로 제공된다.

<figure class="flex flex-col items-center justify-center">
    <figcaption style="text-align: center;"></figcaption>
    <img src="../img/grid-qgis.png" title="국가지점번호 도형">
</figure>

위 그림은 100KM, 10KM, 1KM 격자를 시각화한 결과다. 격자의 위치를 통해 도로가 없는 지역의 위치를 파악할 수 있다.

## 도로명이 부여된 도로 도형

도로명이 부여된 도로구간, 실폭도로, 기초구간의 도형을 제공한다. 모든 데이터는 시도별로 구분되고 `SHP` 파일로 제공된다.
...
**도로구간**은 도로명을 부여하기 위해 설정하는 도로의 시작지점과 끝지점을 말하며, 선형으로 작성된다. **실폭도로**는 면형으로 작성되고어 도로의 폭을 나타낸다. **기초구간**은 도로구간을 일정한 기초간격으로 나눈 선형을 말한다.

<figure class="flex flex-col items-center justify-center">
    <figcaption style="text-align: center;"></figcaption>
    <img src="../img/road-qgis.png" title="국가지점번호 도형">
</figure>

위 그림은 도로명이 부여된 도로구간을 시각화한 결과다. 남색의 면형은 도로구간의 실폭을 나타내는 실폭도로이고, 파란색의 선형은 기초구간을 나타낸다. 흰색은 도로구간으로 도로명이 부여된 도로를 나타낸다.

## 구역의 도형

구역의 도형은 법정구역, 행정구역, 기초구역, 지점번호표기의무지역의 경계를 제공한다. 모든 데이터는 시도별로 구분되고 `SHP` 파일로 제공된다.
...
- 법정구역에 대한 도형정보는 법정구역시도, 법정구역시군구, 법정구역읍면동, 법정구역리로 구분하여 제공된다.

<figure class="flex flex-col items-center justify-center">
    <figcaption style="text-align: center;"></figcaption>
    <img src="../img/area-1-qgis.png" title="구역의 도형">
</figure>

<br>

- 행정구역에 대한 도형정보는 행정구역(행정동)으로 제공된다.
- 기초구역번호(우편번호)를 부여하는 기준인 기초구역에 대한 도형정보는 기초구역으로 제공된다.
- 지점번호표기 의무지역은 도로가 없는 곳의 위치를 식별하기 위해 지점번호표기 의무지역의 경계를 제공한다.
<figure class="flex flex-col items-center justify-center">
    <figcaption style="text-align: center;"></figcaption>
    <img src="../img/area-2-qgis.png" title="구역의 도형">
</figure>

## 기타자료

도로명주소의 배경지공원, 철도, 교량, 하천 등 도로명주소 전자지도의 배경으로 활용되는 도형(점,선,면)을 제공한다. 모든 데이터는 시도별로 구분되고 `SHP` 파일로 제공된다.
...
<figure class="flex flex-col items-center justify-center">
    <figcaption style="text-align: center;"></figcaption>
    <img src="../img/etc-qgis.png" title="기타자료">
</figure>

위 그림은 공원과 지하철선로/지하철역사를 시각화한 결과다. 공원은 면형으로 제공되며, 지하철선로는 선형, 지하철역사는 점형으로 제공된다.
