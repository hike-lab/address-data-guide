...
keywords: [주소정보누리집, 출입구정보, 사물주소, 기초번호, 국가지점번호, 도형, 도로명, 사물마스터]
...
제공하는 주소는 공간데이터(SHP) 또는 좌표데이터(TXT)로 제공된다. 이번 장에서 공간데이터는 시각화를 통해 데이터를 설명하고, 좌표데이터는 예시를 통해 데이터를 설명한다. 각 데이터의 세부 내용은 제공하는 주소의 데이터 레이아웃과 활용가이드문서를 참고한다.
