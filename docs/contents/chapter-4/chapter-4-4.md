...
여러 개의 읍면동을 지나는 경우, '읍면동일련번호'컬럼에서 '00', '01', '02' 등의 구분 코드로 표현한다. 읍면동일련번호가 '00'인 경우가 주도로이므로 이 도로만 남기고 삭제한다.
...
동일한 도로명이 전국의 여러 도로에 부여되고 있다는 것은 앞서 4.3의 분석2에서 확인하였고 이는 도로명이 각 지자체에서 개별적으로 부여하기 때문이다. 중복으로 부여된 도로명의 현황을 다시 한 번 확인하고 시각화를 진행해보겠다. 코드는 도로명 당 부여된 도로명코드의 개수를 groupby를 통해 확인하고, 2개 이상 사용된 도로명만 남기는 순으로 작성한다.
...
첫 번째로 확인할 점은 '도로명이 고유값인지'에 대한 것이다. 앞서 '중앙로'와 같은 중복 도로가 전국에 여러 개 있음을 확인했듯이, 도로명은 고유하지 않다. 따라서 도로명 데이터를 분석할 때, '도로명'값을 식별자로 사용할 수 없다.
...
도로명과 도로명코드가 다르지만 동일한 도로명번호를 갖는 케이스는 총 921건이 있다. 예를들어, 도로명번호 '3265076'는 '충청북도 제천시 제2바이오밸리로', '충청남도 당진시 옥수로', '충청남도 서산시 정자동1로' 세 개의 도로명이 공유하여 사용하고 있다. 도로명번호는 각 지자체에서 부여하기 때문에 상이한 지자체에서 부여한 경우 도로명번호는 중복될 수 있다. 이를 통해 도로명번호는 고유하지 않으며, 식별자로 사용할 수 없음을 알 수 있다.

### 도로명코드는 고유한가?

도로명코드는 시군구코드(5자리)와 도로명번호(7자리)를 합한 값이다. 즉, 동일한 도로명번호를 가져도 서로 다른 시군구에 위치한다면 도로명코드로 구분할 수 있음을 의미한다. 이때, 같은 시군구에 위치하고 동일한 도로명번호는 갖는 케이스가 존재하는지 아래 코드를 통해 확인해보면, 현재 이러한 경우는 존재하지 않는다. 즉, 도로명코드는 도로명 데이터의 PK(Primary Key)로써 고유하며, 중복된 값이 부여될 순 없다는 것을 알 수 있다.
