...
API 데이터에서 시군구코드 값을 포함하는 컬럼은 admCd입니다. 그런데 이 컬럼의 값은 시군구코드 값을 포함할 뿐만 아니라 행정동까지 식별하는 행정동코드를 의미하기 때문에 시군구까지만 나타내는 앞의 5자리까지 자르고, 00000을 붙여서 시군구코드 10자리를 만들어 `search_code`변수에 담아줍니다. 
