...
5장에서는 Python과 Flask를 이용하여 API 데이터를 호출해오는 간단한 웹 서버를 구축하는 과정을 다룹니다. API를 활용하여 외부 서버의 데이터를 요청하고, 우리만의 웹 서버를 만들고, 우리의 서버에서 외부 API 데이터를 활용할 수 있습니다. 실습은 Visual Studio code를 사용하여 진행합니다.
...
- 2. Flask로 웹 서버 구축하기<br>
    웹 애플리케이션 프레임워크인 Flask를 활용하여 간단한 웹 서버를 구축하는 방법을 안내합니다. 내고장알리미 데이터와 주소 검색 API를 활용하여 주소를 검색하면 그 주소에 대한 행정구역 정보를 제공하는 웹 서버를 구축하는 실습을 진행합니다.
